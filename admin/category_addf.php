...
if($_SESSION['username'] == '' ){
		...
		$Q = mysql_query("SELECT `tag_id` FROM `gb_tags` WHERE added_by='admin'");
		 $Q_rows=mysql_num_rows($Q);
	...
	$Q2 = mysql_query("SELECT gb_tags_relation.tag_id FROM gb_tags_relation
LEFT JOIN gb_tags
...
$column="select * ";
$sql=" from gb_tags where  tag_id NOT IN (".$string_groupId.") AND tag_id NOT IN (".$string_groupId2.")";
if($_GET['label_name']!=''){
  ...
  <tr>
    ...
    <td width="93%"><table width="97%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#B1B1B1" style="margin:24px 0 0 5px;">
      ...
      <tr>
		...
		<? include("listadmintags.inc.php");?>
		<BR><BR><BR>

<? include("listpoptags.inc.php");?>
...
<form name="frm_del" method="post" >
  ...
  <table width="54%" border="0" cellspacing="1" cellpadding="4" align=center  bgcolor="#D8D8D8">
    ...
    <tr>
<tr>
  <TD style="font-size:16px">All  Tags</TD>
</tr>
