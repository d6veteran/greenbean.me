...
if($_GET['order_by']=='') $order_by="update_date";
if($_GET['order_by2']=='') $order_by2="desc";
$column="select * ";
$sql=" from fd_business_update ";

$where = " where 1=1 ";

if($_GET['business_name']!=''){
$business_name=$_GET['business_name'];
$search_bussi_name=" left join fd_business on fd_business_update.business_id=fd_business.business_id where fd_business.business_name like '%$business_name%'";	
...
if(comb=='Delete'){
	...
	{
		frmobj.action = "business_update_del.php";
		frmobj.submit();
	}
	else{ 
	return false;
...
<TD width="18%" class='header_bg'>Business name</TD>
<TD width="33%" class='header_bg'>Update Description</TD>
<TD width="8%" class='header_bg'>Update Image</TD>

<!--<TD class='header_bg'>Business add date</TD>-->
<TD width="13%" class='header_bg'>Update Date</TD>


<td width="7%" align="center" class='header_bg'> 
...
?>
     ...
     <TD><?=substr($line[update_description],0,120)?></TD>
     <TD><img src="<?php echo "http://ajaxdesigners.com/frendli/uploads/user/".$line[business_update_img];?>" alt="" width="48" height="48" border="0"></TD>
     <TD><?=date("d M, Y",strtotime("$line[update_date]"))?></TD>
