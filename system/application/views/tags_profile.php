...
<div class="content-md">
  ...
  <h1 style="padding-left:80px"><?=$this->functions->show_tags_name($tag_id);?></h1>
  
  <? if(!isset($_SESSION['userid'])){?>
  <div class="cmt-wp" style=" padding-top:10px; padding-bottom:10px;">
<span>
<a class='login_box' href="#user_login_box">Log in to
request Green Beans for this tag</a>.
</span>
</div>
<? }else {?>
   <div class="cmt-wp" style=" padding-top:10px; padding-bottom:10px;">
<span>
<a href="<?=base_url()?>user/profile/<?=$_SESSION['userid']?>">Request More Green
Beans for this tag.</a>
</span>
</div>
...
<? 
$user_info=$this->functions->getUserInfo($value->user_userid); 

if($user_info['user_photo']!=""){?>
<img src="<?=base_url();?>public/checkresizeimg.php?src=user_image/<?=$user_info['user_photo']?>&w=35&h=35&zc=1" border="0" />
<? }else{ ?>
...
<div class="vote-txt">
  <h2><a href="<?=base_url();?>user/profile/<?=$user_info['user_userid']?>"><?=$user_info['user_username']?></a></h2>
...
?> - <?
 ...
 ?>  
 <a href="<?=base_url()?>tags/<?=$tag_array[$i]['tag_name']?>/0" id="<?=$tag_array[$i]['tag_id']?>">#<?=$tag_array[$i]['tag_name']?></a>
 <? }}?>
 </span>
 </p>
...
<div class="leaderboard-li">
  <p>
  <?
$rank_array=$this->functions->rank_bytags($tag_id);
$rank_array_bytags=$this->functions->rank_on_tags_profile_inlocation($tag_id);
...
?>
  ...
  ?> 
   ...
   in <a href="<?=base_url()?>tags/<?=$this->functions->show_tags_name($tag_id);?>/0"><?=$this->functions->show_tags_name($tag_id);?></a></p>
</div>
...
?>
<div class="leaderboard-li">
  <p><?=$rank_array_bytags[$i]['rank']?> in <a href="<?=base_url()?>locations/<?=$rank_array_bytags[$i]['geo_location_id']?>/0"><?
	$location_array=$this->functions->show_location_name($rank_array_bytags[$i]['geo_location_id']);
	
	echo $location_array[0]['region_name']. ", ". $location_array[0]['city'];
...
<div class="leaderboard-inner">
<div class="leaderboard-li">
  <p><?=$this->functions->bean_earnd_bytags($tag_id);?> Beans Earned</p>
</div>
<div class="leaderboard-li">
  <p><?=$this->functions->bean_requested_bytags($tag_id);?> Bean Requests</p>
</div>
</div>
...
<div class="leaderboard-inner">
 ...
 foreach ($tags as $value)
  {?> 
<div class="leaderboard-li">
  <p><?=$this->functions->count_all("gb_tags_relation","tag_id","tag_id=$value->tag_id");?> <a href="<?=base_url();?>tags/<?=$value->label;?>/0"><?=$value->label;?></a></p>
</div>
 <? }}?>   
