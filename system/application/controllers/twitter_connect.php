...
class Twitter_connect extends Controller {
	...
	public function index()
	{
		...
		$consumer_key = 'BRjTVLMYpVefYkW1jNpQqw';
		$consumer_key_secret = 'sh9jtsD5iOlo90lV0bY82jnPyJBZgzAtInFBrvijhU';
		...
		if ( isset($auth['access_token']) && isset($auth['access_token_secret']) )
		{
			...
			$this->session->set_userdata('twitter_id', $auth['user_id']);
		...
		$this->session->set_userdata('twitter_userinfo', $twitter_userinfo);
		
		if(isset($_SESSION['twitter_id'])){
		header("location: ".base_url()."user/twitter_reg");
		}
